...
class QDA(Classifier):
    ...
    def getClassProbs(self,X):
        ...
        for y in range(self.cardY):
            try:
                py[:,y]= multivariate_normal.pdf(X,mean=self.mu_y[y],cov= self.cov_y[y])
            except:# cov matrix is not positive definite: correct it
                #TODO: fix this issue properly (the correction could be to strong)
                py[:, y] = multivariate_normal.pdf(X, mean=self.mu_y[y], cov=self.cov_y[y]+1)
    ...
    def computeStatistics(self,X,Y,size= 1):
        ...
        self.stats= cm.CondMoments(self.n,[],self.cardY,[tuple([i for i in range(self.n+1)])])
